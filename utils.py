...
def gap_get_result(output_path: str, verbose: bool = False, keep_result: bool = False, board: bool = False,timeout: int = 300):

    output={"output":[-1,-1,-1,-1],"correct":False}
    print("Building ...")

    with open(output_path/"gap_output.json","wb") as logfile:
        output1 = subprocess.Popen(["./run_gap9_match.sh",output_path,"board" if board else "gvsoc"],
